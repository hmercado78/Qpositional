...
from qgis.PyQt.QtWidgets import QGraphicsLineItem, QGraphicsEllipseItem, QGraphicsSimpleTextItem, QGraphicsRectItem, QGraphicsPixmapItem
from qgis.PyQt.QtGui import QPen, QColor, QGradient, QBrush, QRadialGradient, QPixmap
import collections
import numpy as np
...
class QpositionalDialog(QtWidgets.QDialog, FORM_CLASS):
        ...
        for x in range((len(Layer_E))):
            if Layer_E[x].geometryType()==0: # si la capa es  ######## PUNTO ######
                ...
                if Layer_F[x].geometryType()==0: # si la capa de referencia es  ######## PUNTO ######
                    dist2 = processing.run("native:shortestline", {'SOURCE':sel['OUTPUT'],'DESTINATION':Layer_F[x],'METHOD':0,'NEIGHBORS':1,'DISTANCE':None,'OUTPUT':'TEMPORARY_OUTPUT'})
                    project.addMapLayer(dist2['OUTPUT'], True)
                    dist = processing.run("native:extractbylocation", {'INPUT':dist2['OUTPUT'],'PREDICATE':[6],'INTERSECT':ext,'OUTPUT':'TEMPORARY_OUTPUT'}, context=context)
                    #project.removeMapLayer(dist2['OUTPUT'].id())
                ...
                if Layer_F[x].geometryType()==2: # si la capa de referencia es  ######## POLIGONO ######
                    ...
                    dist2 = processing.run("native:shortestline", {'SOURCE':sel['OUTPUT'],'DESTINATION':cen_f['OUTPUT'],'METHOD':0,'NEIGHBORS':1,'DISTANCE':None,'OUTPUT':'TEMPORARY_OUTPUT'})
                    project.addMapLayer(dist2['OUTPUT'], True)
                    dist = processing.run("native:extractbylocation", {'INPUT':dist2['OUTPUT'],'PREDICATE':[6],'INTERSECT':ext,'OUTPUT':'TEMPORARY_OUTPUT'}, context=context)
                    #project.removeMapLayer(dist2['OUTPUT'].id())
                    migrupo.addLayer(dist['OUTPUT'])
    ...
    def hist_mod(self):
        ...
        plot.figure()
        plot.hist(x=lis_mod, bins=extremos, color='#F2AB6D', rwidth=0.85)
        plot.title('Module Histogram')
        plot.xlabel('Distance')
        plot.ylabel('Frequence')
        plot.xticks(extremos)

        img_buf = io.BytesIO()
        im=plot.savefig(img_buf, format='png')

        pixmap = QPixmap(im)
        pixmap_c = QGraphicsPixmapItem(pixmap)
        im_c = Image.open(img_buf)
        im_c.show(title="My Image")

        self.circular.clear()
        self.circular.addItem(pixmap_c)
        text2 = QGraphicsSimpleTextItem(str("hola"))
        self.circular.addItem(text2)
